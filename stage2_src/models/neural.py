...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None,
                layer_cache=None, type=None, predefined_graph_1=None,tar='src'):
        """
        ...
        if layer_cache is not None:
            if type == "self":
                ...
                if tar =='src':
                    ...
                    key_name = "ic_keys"
                    value_name = "ic_values"

                if layer_cache is not None:
                    if layer_cache[key_name] is None:
                        key, value = self.linear_keys(key), \
                                     self.linear_values(value)
                        key = shape(key)
                        value = shape(value)
                    else:
                        key, value = layer_cache[key_name], \
                                     layer_cache[value_name]
                    layer_cache[key_name] = key
                    layer_cache[value_name] = value
                else:
                    key, value = self.linear_keys(key), \
                                 self.linear_values(value)
                    key = shape(key)
