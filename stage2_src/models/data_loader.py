...
class Batch(object):
    def _pad(self, data, pad_id, width=-1):
        ...
        return rtn_data
    def _get_image_data(self,image_hash, is_test ):
        if is_test:
            corpus_type='test'
        else:
            corpus_type = 'train'
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            pre_ic=[x[6] for x in data]
            pre_segs_ic=[x[7] for x in data]
            pre_clss_ic=[x[8] for x in data]
            ...
            mask_src = ~(src == 0)
            mask_tgt = ~(tgt == 0)
            ic = torch.tensor(self._pad(pre_ic, 0))

            segs_ic = torch.tensor(self._pad(pre_segs_ic, 0))
            mask_ic = ~(ic == 0)
            ...
            clss_ic =torch.tensor(self._pad(pre_clss_ic, -1))

            mask_cls_ic = ~(clss_ic == -1)
            clss_ic[clss_ic == -1] = 0

            image_hash = [x[9] for x in data]
            image_data = []
            # for d in image_hash:
            #     _temp_image_list = []
            #
            #     for _img_hash in d:
            #         _image = self._get_image_data(_img_hash, is_test)
            #         _temp_image_list.append(_image)
            #     # print(torch.cat(_temp_image_list,0).unsqueeze(0).shape)
            #     pad_data = torch.zeros(_temp_image_list[0].shape)
            #     if len(_temp_image_list)<clss_ic.shape[1]:
            #         _temp_image_list+=[pad_data]*(clss_ic.shape[1]-len(_temp_image_list))
            ...
            setattr(self,'image_data',image_data)
            setattr(self, 'src', src.to(device))
            setattr(self, 'tgt', tgt.to(device))
            setattr(self, 'segs', segs.to(device))
            setattr(self, 'mask_src', mask_src.to(device))
            setattr(self, 'mask_tgt', mask_tgt.to(device))
            setattr(self, 'ic', ic.to(device))
            setattr(self, 'segs_ic', segs_ic.to(device))
            setattr(self, 'clss_ic', clss_ic.to(device))
            setattr(self, 'mask_ic', mask_ic.to(device))
            setattr(self, 'mask_clss_ic', mask_cls_ic.to(device))
            if (is_test):
                hash_code = [x[5] for x in data]
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        if(is_test):
            return src, tgt, segs, clss, src_sent_labels,hash_code,ic,segs_ic,clss_ic,image_hash,src_txt,tgt_txt
        else:
            return src, tgt, segs, clss, src_sent_labels,hash_code,ic,segs_ic,clss_ic,image_hash
